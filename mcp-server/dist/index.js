...
const db_turso_1 = require("./lib/db-turso");
const db_turso_2 = require("./lib/db-turso");
const db_turso_3 = require("./lib/db-turso");
// Environment variables validation with production support
const envSchema = zod_1.z.object({
    PROJECT_ID: zod_1.z.string().optional(),
    API_TOKEN: zod_1.z.string().optional(),
    PORT: zod_1.z.string().transform(Number).default('3001'),
    LOG_LEVEL: zod_1.z.enum(['error', 'warn', 'info', 'debug']).default('info'),
    LOG_REQUESTS: zod_1.z.string().transform(val => val === 'true').default('false'),
    ...
    ENABLE_CACHING: zod_1.z.string().transform(val => val === 'true').default('false'),
    CACHE_TTL: zod_1.z.string().transform(Number).default('300'),
    MAX_CONNECTIONS: zod_1.z.string().transform(Number).default('100'),
...
const requestTimes = [];
function trackRequest(startTime, isError = false) {
...
function checkAlertThresholds(responseTime, isError) {
...
function updateHealthCheck(component, status, latency, error) {
...
server.addTool({
    ...
    parameters: zod_1.z.object({
        include_detailed_checks: zod_1.z.boolean().optional().describe('Include detailed subsystem health checks'),
        check_database_performance: zod_1.z.boolean().optional().describe('Include database performance metrics')
    }),
    execute: async ({ include_detailed_checks = false, check_database_performance = false }) => {
        ...
        try {
            ...
            try {
                ...
                const projects = await (0, db_turso_1.getProjects)();
...
server.addTool({
    ...
    parameters: zod_1.z.object({
        api_token: zod_1.z.string().describe('API token to validate')
    }),
    execute: async ({ api_token }) => {
        const startTime = Date.now();
        try {
            const project = await (0, db_turso_3.getProjectByApiKey)(api_token);
            if (!project) {
...
server.addTool({
    ...
    parameters: zod_1.z.object({
        project_id: zod_1.z.string().describe('Project ID to retrieve')
    }),
    execute: async ({ project_id }) => {
        try {
            const project = await (0, db_turso_1.getProject)(project_id);
            if (!project) {
                return JSON.stringify({
                    success: false,
...
server.addTool({
    ...
    execute: async ({ name, description = '' }) => {
        try {
            const project = await (0, db_turso_1.createProject)(name, description);
            return JSON.stringify({
                success: true,
                project: {
                    id: project.id,
...
server.addTool({
    ...
    parameters: zod_1.z.object({
        project_id: zod_1.z.string().describe('Project ID to get logs for')
    }),
    execute: async ({ project_id }) => {
        try {
            ...
            const logs = await (0, db_turso_2.getProjectLogs)(project_id);
            return JSON.stringify({
                success: true,
                project_id,
                logs: logs.map(log => ({
...
server.addTool({
    ...
    parameters: zod_1.z.object({
        log_id: zod_1.z.string().describe('Log ID to retrieve content for')
    }),
    execute: async ({ log_id }) => {
        try {
            const log = await (0, db_turso_2.getLog)(log_id);
            if (!log) {
                return JSON.stringify({
                    success: false,
                    error: 'Log not found',
...
server.addTool({
    ...
    parameters: zod_1.z.object({
        project_id: zod_1.z.string().describe('Project ID to create log for'),
        content: zod_1.z.string().describe('Log content'),
        comment: zod_1.z.string().optional().describe('Optional comment for the log entry')
    }),
    execute: async ({ project_id, content, comment = '' }) => {
        try {
            ...
            const log = await (0, db_turso_2.createLog)(project_id, content, comment);
            return JSON.stringify({
                success: true,
                log: {
                    id: log.id,
...
server.addTool({
    ...
    parameters: zod_1.z.object({
        include_trends: zod_1.z.boolean().optional().describe('Include performance trend analysis'),
        include_alerts: zod_1.z.boolean().optional().describe('Include active alerts and thresholds')
    }),
...
server.addTool({
    ...
    parameters: zod_1.z.object({
        severity_filter: zod_1.z.enum(['all', 'degraded', 'failed']).optional().describe('Filter alerts by severity level'),
        include_resolved: zod_1.z.boolean().optional().describe('Include recently resolved alerts')
    }),
    execute: async ({ severity_filter = 'all', include_resolved = false }) => {
        ...
        try {
            ...
            if (errorRate > metrics.alertThresholds.errorRate && metrics.requestCount >= 10) {
                systemAlerts.push({
                    component: 'error_rate',
                    status: errorRate > 15 ? 'failed' : 'degraded',
            ...
            const allAlertsWithSystem = [...filteredAlerts, ...systemAlerts.filter(alert => severity_filter === 'all' || alert.status === severity_filter)];
            trackRequest(startTime);
            return JSON.stringify({
                success: true,
                timestamp: new Date().toISOString(),
...
server.addTool({
    ...
    parameters: zod_1.z.object({
        error_rate: zod_1.z.number().min(1).max(50).optional().describe('Error rate percentage threshold (1-50)'),
        response_time: zod_1.z.number().min(100).max(10000).optional().describe('Response time threshold in milliseconds (100-10000)'),
        consecutive_errors: zod_1.z.number().min(1).max(10).optional().describe('Consecutive errors threshold (1-10)'),
        db_failure_timeout: zod_1.z.number().min(5000).max(300000).optional().describe('Database failure timeout in milliseconds (5000-300000)')
    }),
    execute: async ({ error_rate, response_time, consecutive_errors, db_failure_timeout }) => {
        ...
        try {
            ...
            if (error_rate !== undefined)
                metrics.alertThresholds.errorRate = error_rate;
            if (response_time !== undefined)
                metrics.alertThresholds.responseTime = response_time;
            if (consecutive_errors !== undefined)
                metrics.alertThresholds.consecutiveErrors = consecutive_errors;
            if (db_failure_timeout !== undefined)
                metrics.alertThresholds.dbFailureTimeout = db_failure_timeout;
...
if (isProduction && env.HEALTH_CHECK_INTERVAL > 0) {
    setInterval(async () => {
        try {
            ...
            const dbStart = Date.now();
            await (0, db_turso_1.getProjects)();
...
server.addTool({
    ...
    parameters: zod_1.z.object({
        project_id: zod_1.z.string().describe('Project ID to search logs in'),
        search_query: zod_1.z.string().optional().describe('Text to search for in messages and details'),
        levels: zod_1.z.string().optional().describe('Comma-separated log levels to include (LOG,ERROR,INFO,WARN,DEBUG)'),
        tags: zod_1.z.string().optional().describe('Comma-separated tags to filter by'),
        time_from: zod_1.z.string().optional().describe('Start time filter (ISO string or relative like "1h", "30m")'),
        time_to: zod_1.z.string().optional().describe('End time filter (ISO string)'),
        exclude_extended_data: zod_1.z.boolean().optional().default(false).describe('Exclude _extended field from details for cleaner output'),
        verbosity: zod_1.z.enum(['titles', 'summary', 'full']).optional().default('summary').describe('Output detail level'),
        limit: zod_1.z.number().min(1).max(1000).optional().default(50).describe('Maximum number of entries to return'),
        context_lines: zod_1.z.number().min(0).max(10).optional().describe('Include N lines before/after each match for context')
    }),
    execute: async ({ project_id, search_query, levels, tags, time_from, time_to, exclude_extended_data = false, verbosity = 'summary', limit = 50, context_lines }) => {
        const startTime = Date.now();
        try {
            // Verify project exists
            const project = await (0, db_turso_1.getProject)(project_id);
            ...
            for (const log of logs) {
                if (!log.content) {
                    ...
                    const fullLog = await (0, db_turso_2.getLog)(log.id);
                    if (!fullLog?.content)
                        continue;
                ...
                for (let i = 0; i < lines.length; i++) {
                    ...
                    if (match) {
                        ...
                        if (detailsStr) {
                            try {
                                ...
                                if (details && typeof details === 'object' && Array.isArray(details._tags)) {
                                    entryTags = details._tags.filter((tag) => typeof tag === 'string');
                                }
                        ...
                        if (matches && tagFilter && entryTags.length > 0) {
                            const hasMatchingTag = tagFilter.some(filterTag => entryTags.some(entryTag => entryTag.includes(filterTag)));
                            if (!hasMatchingTag)
                                matches = false;
                        }
                        // Time filter
                        if (matches && (timeFrom || timeTo)) {
                            try {
                                ...
                                if (timeFrom && entryTime < timeFrom)
                                    matches = false;
                                if (timeTo && entryTime > timeTo)
                                    matches = false;
                            }
                            catch {
...
server.addTool({
    ...
    parameters: zod_1.z.object({
        project_id: zod_1.z.string().describe('Project ID to get entries from'),
        limit: zod_1.z.number().min(1).max(100).optional().default(20).describe('Number of recent entries to return'),
        levels: zod_1.z.string().optional().describe('Comma-separated log levels to include (LOG,ERROR,INFO,WARN,DEBUG)'),
        exclude_debug: zod_1.z.boolean().optional().default(false).describe('Exclude DEBUG level entries')
    }),
