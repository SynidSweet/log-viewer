...
const { LogEntryDetails } = require('../../../../src/components/log-viewer/log-entry-details.tsx');

describe('LogEntryDetails', () => {
  let renderResult: RenderResult;

  beforeEach(() => {
    renderResult = render(<LogEntryDetails />);
  });
  ...
  it('should have correct TypeScript props', () => {
    ...
    const { container } = render(<LogEntryDetails />);
    ...
    commonProps.forEach((props, index) => {
      try {
        const { unmount } = render(<LogEntryDetails {...props} />);
