...
const { LogItem } = require('../../../../src/components/log-viewer/log-item.tsx');

describe('LogItem', () => {
  let renderResult: RenderResult;

  beforeEach(() => {
    renderResult = render(<LogItem />);
  });
  ...
  it('should have correct TypeScript props', () => {
    ...
    const { container } = render(<LogItem />);
    ...
    commonProps.forEach((props, index) => {
      try {
        const { unmount } = render(<LogItem {...props} />);
