...
const { PageSession } = require('../../../src/components/page-session.tsx');

describe('PageSession', () => {
  let renderResult: RenderResult;

  beforeEach(() => {
    renderResult = render(<PageSession />);
  });
  ...
  it('should have correct TypeScript props', () => {
    ...
    const { container } = render(<PageSession />);
    ...
    commonProps.forEach((props, index) => {
      try {
        const { unmount } = render(<PageSession {...props} />);
