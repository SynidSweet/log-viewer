...
const { UploadLogsModal } = require('../../../src/components/upload-logs-modal.tsx');

describe('UploadLogsModal', () => {
  let renderResult: RenderResult;

  beforeEach(() => {
    renderResult = render(<UploadLogsModal />);
  });
  ...
  it('should have correct TypeScript props', () => {
    ...
    const { container } = render(<UploadLogsModal />);
    ...
    commonProps.forEach((props, index) => {
      try {
        const { unmount } = render(<UploadLogsModal {...props} />);
