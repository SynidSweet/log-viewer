...
const { EditProjectModal } = require('../../../src/components/edit-project-modal.tsx');

describe('EditProjectModal', () => {
  let renderResult: RenderResult;

  beforeEach(() => {
    renderResult = render(<EditProjectModal />);
  });
  ...
  it('should have correct TypeScript props', () => {
    ...
    const { container } = render(<EditProjectModal />);
    ...
    commonProps.forEach((props, index) => {
      try {
        const { unmount } = render(<EditProjectModal {...props} />);
